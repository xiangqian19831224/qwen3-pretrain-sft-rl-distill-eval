...
class DPOChatbot:
    def __init__(self, sft_merged_model_path, dpo_adapter_path):
        self.device = self._get_device()
        self.sft_merged_model_path = sft_merged_model_path
        self.ppo_adapter_path = dpo_adapter_path
    ...
    def load_model(self):
        ...
        print(f"--> DPO适配器: {self.ppo_adapter_path}")
...
def test_model(chatbot: DPOChatbot, output_file: str):
    """批量测试PPO模型效果"""
    test_questions = [
    ...
    print(f"\nDPO测试完成！结果已保存到 {output_file}")

def interactive_chat(chatbot: DPOChatbot):
    """PPO模型交互式对话"""
    print("\n" + "="*80 + "\nDPO模型交互式对话\n" + "="*80)
...
def main():
    ...
    parser.add_argument("--adapter_path", type=str, required=True, help="PPO LoRA适配器的路径 (例如 ./output/dpo_adapter)")
    ...
    chatbot = DPOChatbot(args.model_path, args.adapter_path)
    chatbot.load_model()
